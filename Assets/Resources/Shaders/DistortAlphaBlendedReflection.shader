...
Shader "Custom/DistortAlphaBlendedReflection" {
Properties {
	...
	_ReflectionTex ("Ref Texture", 2D) = "white" {}
	_ReflectionDistortedDistortion ("Ref Distortion" ,range(-1,1)) = 0.02
}
/************************************************************************************************************************/
/************************************************************************************************************************/
Category {
...
//#pragma target 3.0
		...
		float _ReflectionDistortedDistortion;
		...
		v2f vert (appdata_t v)
		{
			...
			o.uv_ref = ComputeScreenPos(o.vertex);
			return o;
		}
		...
		fixed4 frag( v2f i ) : COLOR
		{
			...
			fixed4 finalColor = tex2D( _MainTex, i.uvmain);

			float4 ffuv = UNITY_PROJ_COORD(i.uv_ref);
			float aa = (finalColor.r +finalColor.g +finalColor.b)/3;
			ffuv.xy += aa * _ReflectionDistortedDistortion;
			half4 refl_col = tex2Dproj( _ReflectionTex, ffuv );
			finalColor.rgb = lerp( finalColor.rgb, refl_col.rgb, refl_col.a );

			finalColor = 2.0f * i.color * _TintColor * finalColor * _BrightnessRatio;
